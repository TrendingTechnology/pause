...
Linux のインストール自体はかんたん。iso ファイルをダウンロードして、USBメモリに [rufus](http://rufus.akeo.ie/) を使って書き込んで、そのUSBメモリからブートすればOK。ブート方法は `F2` で BIOS に入って `F8`、もしく `ESC + F12` でもいけるっぽい。
...
うーん、ということで Ubuntu 17 を入れてみると、上記の不具合は解消。でもやはり全体的にちょっと重いので、軽量なものを再インストールすることにしました。

## Lubuntu 17.10 に落ち着いた

軽量 Linux ということで [Ubuntu Mate](https://ubuntu-mate.org), [Xubuntu](https://xubuntu.org) も試してみたけど、いちばん軽そうな [Lubuntu](https://lubuntu.me) に落ち着きました。見た目は味気なくなるけど、まあそんなリッチな画面は求めてないし。重さはだいぶマシになってます。

や、自分の使い方だと、Linux でもあんまり困らないな〜。最初は Chromebook系でもいいかとか思ってたくらいで(ローカル開発環境にちょっと不安があったので、やめましたが)

- ブラウザとターミナルとエディタがあれば、自分的にはだいたい十分
